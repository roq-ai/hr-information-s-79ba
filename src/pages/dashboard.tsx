...
import { JobPositionsListPage as ShowCaseList } from 'pages/job-positions';
...
function DashboardPage() {
  ...
  const jobPositionsStat = useSWR<PaginatedInterface<JobPositionsInterface>>(
    () => `/job-positions`,
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Job Positions',
      result: jobPositionsStat?.data?.totalCount,
      isLoading: jobPositionsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Employees',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`job-positions`} passHref legacyBehavior>
                  <Button
